...
tmr.setInput('listedGroups', 'mock_list_groups');
...
let a: ma.TaskLibAnswers = <ma.TaskLibAnswers>{
    "exec": {
        "java -cp test_full_path com.microsoft.alm.Driver -s https://example.test/v0.1 -u test_username -p test_password -c deploy mock_file --force --disabled --name=mock_name --runtime-name=mock_runtimeName --server-groups=mock_list_groups mock_options": {
            "code": 0,
